...
def fetch(url: str, cookies=None, allow_retry=True):
    cookies = cookies or {}
    r = requests.get(url, headers=rand_headers(), cookies=cookies, timeout=45, verify=False)
...
def main():
    ...
    s1.add_argument('--date', required=False, help='Exact start date YYYY-MM-DD (Kitsu startDate) for year filter & date match')
    ...
    try:
        if args.cmd == 'search':
            print(json.dumps(search(args.query, args.date), indent=4))

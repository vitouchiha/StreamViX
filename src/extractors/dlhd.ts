 ...
 * Logic (NO CACHE - ad ogni richiesta):
 * 1. Try direct connection (fast if IP not blocked)
...
async function axiosGetWithRetry(url: string, headers: any, timeout = 15000, responseType?: 'arraybuffer'): Promise<any> {
  ...
  try {
    ...
    for (let i = 0; i < proxies.length && i < 2; i++) {
      ...
      try {
        debugLog(`[Attempt ${3 + i}/4] Trying proxy from proxies.ts...`);
